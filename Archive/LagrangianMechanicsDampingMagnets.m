...
syms EQNmotionT1 EQNmotionT2 b F1x F1y Tau1y F2x F2y Tau2y F3x F3y Tau3y

% Note, Eqns of motion are given by EQNmotionT1 = 0 and EQNmotionT2 = 0

EQNmotionT1 = dt_wrt_dL_wrt_dotT1 - dL_wrt_T1 + b*diff(Theta1) - (F1x*L1*cos(Theta1) + F1y*L1*sin(Theta1)) - Tau1y;
EQNmotionT1 = expand(EQNmotionT1);
EQNmotionT1 = simplify(EQNmotionT1)

EQNmotionT2 = dt_wrt_dL_wrt_dotT2 - dL_wrt_T2 + b*diff(Theta2) - (F2x*L2*cos(Theta2) + F2y*L2*sin(Theta2)) - Tau2y;
EQNmotionT2 = expand(EQNmotionT2);
EQNmotionT2 = simplify(EQNmotionT2)

EQNmotionT3 = dt_wrt_dL_wrt_dotT3 - dL_wrt_T3 + b*diff(Theta3) - (F3x*L3*cos(Theta3) + F3y*L3*sin(Theta3)) - Tau3y;
